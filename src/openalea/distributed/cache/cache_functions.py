...
# def launch(timer, id_exp, index, func, *args, **kwargs):
#     # retreive intermediate data if it exist and execute the act otherwise
#     id_task = get_id2(func, args, kwargs)
#     tmp_path = index.search(timer, id_task)
#     dname = str(func.__name__) + "__" + id_task
#
#     if tmp_path is not None:
#         # si il existe une methode read :
#         if tmp_path.split(".")[-1] == "csv":
#             try:
#                 ttmp = time.time()
#                 tmp_data = read(VoxelGrid, tmp_path, read_method='read')
#                 timer.update_ttotal(ttmp)
#
...
#
#         ttmp = time.time()
#         index.update(timer, id_task, tmp_data, dname, id_exp, dtype="csv")
...
def compute_a_snapshot(plant_snapshot, id_exp, cache_path, index=None, timer=None):
    """
    Execute the WF phenomenal on plant 1
    :param args: id_exp : an id to represent the exp
                overwrite : [NOT WORKING] True to overwrite the index
    :return:
    """

    #if no timer started -> start one
    if timer==None:
        timer = Timer()

    # if no index started -> start one
    if index==None:
        index_path = cache_path + "index.csv"
        index = IndexFile(index_path)
    ...
    # # data used
    # plant_number = 1
    # # with timeit_context('load raw images'):
    # raw_image = raw_images(plant_number=plant_number)
    raw_image = plant_snapshot

    images_bin = launch2(timer, cache_path, index, binarize, raw_image)

    #TODO: change calibration for each plant (get the right one on IRODS)
    calibration = calibrations(plant_number=1)
    image_views = get_image_views(images_bin, calibration)

    # print get_id(reconstruction_3d, image_views)

    voxel_grid = launch2(timer, cache_path, index, reconstruction_3d, image_views, start_voxel_size=256)

    g = launch2(timer, cache_path, index, graph_from_voxel_grid, voxel_grid)

    voxel_skeleton = launch2(timer, cache_path, index, skeletonize, voxel_grid, g, subgraph=None)

    side_image_projection = launch2(timer, cache_path, index, get_side_image_projection_list, images_bin, calibration)

    vs = launch2(timer, cache_path, index, segment_reduction, voxel_skeleton, side_image_projection, tolerance=4, nb_min_pixel=1)

    vms = launch2(timer, cache_path, index, maize_segmentation, vs, g)

    maize_segmented = launch2(timer, cache_path, index, maize_analysis, vms)

    timer.update_ttotal(t0)
...
def compute_a_snapshot_irods(plant, cache_path, index=None, timer=None, irods_session=None):
    """
    Execute the WF phenomenal on plant 1
    ...
    images_bin = launch_irods(timer, plant_name, irods_session, cache_path, index, binarize, raw_image)
    # images_bin = launch_only_exec(timer, plant_name, irods_session, cache_path, index, binarize, raw_image)

    # calibration = calibrations(plant_number=1)
    calibration = plant[1]
    image_views = get_image_views(images_bin, calibration)

    voxel_grid = launch_irods(timer, plant_name, irods_session, cache_path, index, reconstruction_3d, image_views, voxels_size=4, start_voxel_size=4096)
    # voxel_grid = launch_only_exec(timer, plant_name, irods_session, cache_path, index, reconstruction_3d, image_views, start_voxel_size=1024)

    g = launch_irods(timer, plant_name, irods_session, cache_path, index, graph_from_voxel_grid, voxel_grid)
    # g = launch_only_exec(timer, plant_name, irods_session, cache_path, index, graph_from_voxel_grid, voxel_grid)

    voxel_skeleton = launch_irods(timer, plant_name, irods_session, cache_path, index, skeletonize, voxel_grid, g, subgraph=None)
    # voxel_skeleton = launch_only_exec(timer, plant_name, irods_session, cache_path, index, skeletonize, voxel_grid, g, subgraph=None)

    side_image_projection = launch_irods(timer, plant_name, irods_session, cache_path, index, get_side_image_projection_list, images_bin, calibration)
    ...
    vs = launch_irods(timer, plant_name, irods_session, cache_path, index, segment_reduction, voxel_skeleton, side_image_projection, required_visible=4, nb_min_pixel=100)
    # vs = launch_only_exec(timer, plant_name, irods_session, cache_path, index, segment_reduction, voxel_skeleton, side_image_projection, tolerance=4, nb_min_pixel=1)

    vms = launch_irods(timer, plant_name, irods_session, cache_path, index, maize_segmentation, vs, g)
    # vms = launch_only_exec(timer, plant_name, irods_session, cache_path, index, maize_segmentation, vs, g)

    maize_segmented = launch_irods(timer, plant_name, irods_session, cache_path, index, maize_analysis, vms)
    # maize_segmented = launch_only_exec(timer, plant_name, irods_session, cache_path, index, maize_analysis, vms)
