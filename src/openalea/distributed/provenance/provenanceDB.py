...
class ProvCassandra():
    ...
    def remove_all_item(self):
        query = SimpleStatement("""
        TRUNCATE wf_provenance
        """)
        self.client.execute(query)
        query = SimpleStatement("""
        TRUNCATE task_provenance
        """)
        self.client.execute(query)
        return 0

    def delete(self):
        cmd = """DROP TABLE IF EXISTS wf_provenance;
        """
        self.client.execute(cmd)
        cmd = """DROP TABLE IF EXISTS task_provenance;
        """
        self.client.execute(cmd)
...
def start_provdb(provenance_config=None, provenance_type="Files"):
    ...
    if provenance_type == "Cassandra":
        ...
        provdb.init(provenance_config)
        return provdb
    if provenance_type == "NoOpenAlea":
        provdb = ProvCassandraNOOPENALEA()
        provdb.init(provenance_config)
        return provdb
